...
class SelectMoveFrame():
    ...
    def _mouse_press(self, event):
        print("MOUSE PRESSED")
        self._place_mouse(event,press=True)
        self._mouse_is_pressed = True
        self.mouse_released = False
    ...
    def action(self):
        ...
        if self._mouse_is_pressed and self.mouse_released:
            self._mouse_is_pressed = False
            self.mouse_released = False
            return np.array([self._mouse_coords_press[0], 1. - self._mouse_coords_press[1], self._mouse_coords_release[0], 1. - self._mouse_coords_release[1]])
        else:
